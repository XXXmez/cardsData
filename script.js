...
function createCard(list) {
  ...
  pgp.classList.add('good__price');
  
  h2.textContent = list.text;
  img.src = list.imgUrl;
  img.alt = list.text;
  pgp.textContent = list.price + '₽/кг' ;
  if (list.inStock) {
  ...
  card.appendChild(pgp);
  if (list.specialOffer) {
    var p = document.createElement('p');
    p.classList.add('good__special-offer');
    p.textContent = list.specialOffer;
    card.appendChild(p);
  }
